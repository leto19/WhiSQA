...
def mel_filters(device, n_mels: int = N_MELS) -> torch.Tensor:
    ...
    with package_file("data", "mel_filters.npz") as filter_path:
        with np.load(filter_path, allow_pickle=True) as filters_file:
            return torch.from_numpy(filters_file[f"mel_{n_mels}"]).to(device)
