...
if __name__ == "__main__":
    ...
    import timeit
    #in_tensor =  torchaudio.load(sys.argv[1])[0]
    #in_tensor = torch.stack([in_tensor,in_tensor]).squeeze(1).cuda()
    #print(in_tensor.shape)
    #in_tensor.requires_grad = True

    model = WhisperWrapper_full(layer=-1,use_feat_extractor=True).cuda()
    #model2 = WhisperWrapper_encoder_debug(layer=None,use_feat_extractor=False).cuda()
   

    # print(out.shape)
    # print(out.requires_grad)
    # print("------------------")
    # out2,out_feats2 = model2(in_tensor)
    # print(out2.shape)
    # print(out2.requires_grad)
    # print("------------------")x


    # # Plot out_feats and out out_feats2, out2 on one image
    # fig, axs = plt.subplots(2, 1, figsize=(10, 8))
    # axs[0].imshow(np.flipud(out_feats[0].detach().cpu().numpy()[:,:1000]))
    # axs[0].set_title('whisper input mel feats(first 1000 samples)')
    # out_disp = torch.sigmoid(out[0]).detach().cpu().numpy()[:600,:].T*10000
    # axs[1].imshow(out_disp)
    # axs[1].set_title('whisper encoder output(first 600 samples)')
    # # axs[2].imshow(np.flipud(out_feats2[0].detach().cpu().numpy()[:,:1000]))
    # # axs[2].set_title('out_feats2')
    # # out_disp2 = torch.sigmoid(out2[0]).detach().cpu().numpy()[:600,:].T*10000
    # # axs[3].imshow(out_disp2)
    # # axs[3].set_title('out2')
    # plt.tight_layout()
    # plt.savefig("%s_noisy_whisper.png"%sys.argv[1].split("/")[-1].strip(".wav"))

   # in_tensor =  torchaudio.load(sys.argv[1])[0]
   # in_tensor2 = torchaudio.load(sys.argv[1].replace("clean","noisy"))[0]
   # in_tensor = torch.stack([in_tensor,in_tensor2]).squeeze(1).cuda()
    #in_tensor = torch.rand(32,16000*2)
    in_tensor = torch.sin(torch.rand(2,16000*5))
    print(in_tensor.shape)
    in_tensor.requires_grad = True

    #record how long it takes to run the model
    ...
    # axs[0].imshow(np.flipud(out_feats[0].detach().cpu().numpy()[:,:1000]))
    # axs[0].set_title('clean whisper input mel feats(first 1000 samples)')
    # out_disp1 = torch.sigmoid(out[0]).detach().cpu().numpy()[:600,:].T*10000
    # axs[1].imshow(out_disp1)
    # axs[1].set_title('clean whisper encoder output(first 600 samples)')
    # axs[2].imshow(np.flipud(out_feats[1].detach().cpu().numpy()[:,:1000]))
    # axs[2].set_title('noisy whisper input mel feats(first 1000 samples)')
    # out_disp2 = torch.sigmoid(out[1]).detach().cpu().numpy()[:600,:].T*10000
    # axs[3].imshow(out_disp2)
    # # set the y ticks to a range 0 to 3000 in steps of 100
    # axs[3].set_yticks(np.arange(0, 600, 100))
    # axs[3].set_title('noisy whisper encoder output(first 600 samples)')
    # axs[4].imshow((out_disp1 - out_disp2)*10000000)
